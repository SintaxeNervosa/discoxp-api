...
@Component
public class DefaultOrderValidator implements OrderValidator, QuantityValidator, PaymentValidator, FreteValidator, AddressValidator, OrderStatusValidator {
    ...
    @Override
    public void validateChangeOrderStatus(ChangeOrderStatusRequestDTO request, String id) {
        validateStatus(request);

        try {
            Long.parseLong(id);
        } catch (NumberFormatException e) {
            ValidationErrorRegistry.addError("ID inválido");
        }
